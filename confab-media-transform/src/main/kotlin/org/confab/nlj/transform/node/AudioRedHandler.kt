...
import org.confab.nlj.util.RtpPacketCache
import org.confab.rtp.extensions.toHex
import org.confab.rtp.rtp.RedundancyBlockHeader
import org.confab.rtp.rtp.RtpPacket
import org.confab.rtp.rtp.header_extensions.AudioLevelHeaderExtension
import org.confab.rtp.util.RtpUtils.Companion.applySequenceNumberDelta
import org.confab.rtp.util.RtpUtils.Companion.getTimestampDiffAsInt
            ...
            when (config.distance) {
                ...
                RedDistance.TWO -> {
                    getPacketToProtect(
                        ...
                        // when the receiver of the RED stream is another confab-videobridge instance (via Octo),
...
class Config {
    val policy: RedPolicy by config { "jmt.audio.red.policy".from(ConfabConfig.newConfig) }
    val distance: RedDistance by config { "jmt.audio.red.distance".from(ConfabConfig.newConfig) }
    val vadOnly: Boolean by config { "jmt.audio.red.vad-only".from(ConfabConfig.newConfig) }
}
