...
package org.confab.videobridge.stats.config
...
class StatsManagerConfig private constructor() {
    ...
    val interval: Duration by config {
        "org.confab.videobridge.STATISTICS_INTERVAL".from(ConfabConfig.legacyConfig).convertFrom(Duration::ofMillis)
        "videobridge.stats.interval".from(ConfabConfig.newConfig)
    }

    /**
     * The enabled stat transports
     *
     * Note that if 'org.confab.videobridge.STATISTICS_TRANSPORT' is present at all
    ...
    val transportConfigs: List<StatsTransportConfig> by config {
        "org.confab.videobridge."
            .from(ConfabConfig.legacyConfig)
            .convertFrom<Map<String, String>> {
                if ("org.confab.videobridge.STATISTICS_TRANSPORT" in it) {
    ...
    private fun Map<String, String>.toStatsTransportConfig(): List<StatsTransportConfig> {
        val transportTypes =
            this["org.confab.videobridge.STATISTICS_TRANSPORT"]?.split(",") ?: return listOf()
        return transportTypes.mapNotNull { transportType ->
            val interval = this["org.confab.videobridge.STATISTICS_INTERVAL.$transportType"]?.let {
