...
import org.confab.videobridge.JvbLastN
...
class LastNReducer(
    ...
    private val reductionScale: Double by config(
        "${JvbLoadReducer.CONFIG_BASE}.last-n.reduction-scale".from(ConfabConfig.newConfig)
    )

    private val recoverScale: Double by config(
        "${JvbLoadReducer.CONFIG_BASE}.last-n.recover-scale".from(ConfabConfig.newConfig)
    )

    private val impactTime: Duration by config(
        "${JvbLoadReducer.CONFIG_BASE}.last-n.impact-time".from(ConfabConfig.newConfig)
    )

    private val minLastN: Int by config(
        "${JvbLoadReducer.CONFIG_BASE}.last-n.minimum-last-n-value".from(ConfabConfig.newConfig)
    )

    private val maxEnforcedLastN: Int by config(
        "${JvbLoadReducer.CONFIG_BASE}.last-n.maximum-enforced-last-n-value".from(ConfabConfig.newConfig)
    )
