...
#
#   Arguments:
#   "-p"(mandatory) the PID of confab Videobridge process
...
if [ "$pid" = "" ] ;then
    if [ -f /var/run/confab-videobridge.pid ]; then
        pid=`cat /var/run/confab-videobridge.pid`
    else
        pid=`cat /var/run/confab-videobridge/confab-videobridge.pid`
    fi
fi
...
then
    ...
    rm -f /var/run/confab-videobridge.pid
    rm -f /var/run/confab-videobridge/confab-videobridge.pid
