...
condition_egress() {
  ...
  (cat /etc/pf.conf && echo "dummynet-anchor \"confab-videobridge\"" && echo "anchor \"confab-videobridge\"") | sudo pfctl -f -

  # Pipe all UDP traffic from port PORT through the EGRESS_PIPE.
  echo "dummynet out quick proto udp from any port $PORT to any pipe $EGRESS_PIPE" | sudo pfctl -a confab-videobridge -f -
...
condition_ingress() {
  ...
  echo "dummynet out quick proto udp from any to any port $PORT pipe $INGRESS_PIPE" | sudo pfctl -a confab-videobridge -f -
