...
PID_PATH="/var/run/confab-videobridge.pid"
JVB_UID=`id -u jvb`
...
if [ ! -f $PID_PATH ]; then
    PID_PATH="/var/run/confab-videobridge/confab-videobridge.pid"
fi
...
if [ ! -z $RUNNING ]; then
    ...
    tar zcvf jvb-dumps-${STAMP}-${PID}.tgz ${THREADS_FILE} ${HEAP_FILE} ${HEAPDUMP_FILE} ${CRASH_FILES} /var/log/confab/jvb.log /tmp/hs_err_*
    rm ${HEAP_FILE} ${THREADS_FILE}
else
    ls $JVB_HEAPDUMP_PATH >/dev/null 2>&1
    if [ $? -eq 0 ]; then
        echo "JVB not running, but previous heap dump found."
        tar zcvf jvb-dumps-${STAMP}-crash.tgz $JVB_HEAPDUMP_PATH ${CRASH_FILES} /var/log/confab/jvb.log /tmp/hs_err_*
        rm ${JVB_HEAPDUMP_PATH}
    else
        tar zcvf jvb-dumps-${STAMP}-crash.tgz /var/log/confab/jvb.log ${CRASH_FILES} /tmp/hs_err_*
